...
@section('title', 'Pengaturan - Admin BPSDMD')
...
<div class="pengaturan-grid">
  ...
  <!-- ===== FASILITAS ===== -->
  <div class="settings-card span-full">
    <div class="settings-card-header">Fasilitas Gedung</div>
    <div class="settings-card-body">
      <form action="{{ url('/admin/pengaturan/fasilitas/'.($fasilitas->id_fasilitas ?? 1)) }}" method="POST">
        @csrf
        @method('PUT')
        <textarea name="deskripsi_fasilitas" class="wysihtml5 fasilitas-editor">{!! $fasilitas->deskripsi_fasilitas ?? '' !!}</textarea>
